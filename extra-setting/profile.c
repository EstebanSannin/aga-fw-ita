...
//    exit(0);
    ...
    sprintf(strFileToLoad, "profile.html");
	...
	if (strncmp("save", entries[0].val, 4) == 0) {
            int ena = 0;
            if (strcasecmp("true", entries[1].val) == 0) {
        	ena = 1;
	    }	
	    if (entries[2].val[0] == '\0') {
        	SetProfileScript(ena, "\%23");
	    } else {
        	SetProfileScript(ena, entries[2].val);
	    }
	    ...
	    printf("<body>\n");
	    printf("<script>\n");
	    printf("\teval('location = \"profile.cgi\"');\n");
	    printf("</script>\n");
	    printf("</body>\n");
	    printf("</html>\n");
	    return 0;
	...
	if (strcasecmp("GetProfileScript()", tag) == 0) {
	    GetProfileScript();
	}
...
int SetProfileScript(int ArgEnable, char *ArgScript)
{
    ...
    if ((xmlCfgLen + MAX_PROFILE_SECTION_SIZE) > rsiSize) {
	free(strXmlBuf);
	return -3;
    }
    ...
    sprintf(newRsi, "%s<profile>\n<global state=\"%s\" script=\"%s\" />\n</profile>\n",
        newRsi, strEnable, ArgScript);
